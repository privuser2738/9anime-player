...
setTimeout(() => {
  if (typeof VerboseLogger !== 'undefined') {
    ...
    logger.logSystem('Content script loaded', {
...
class PlaylistPlayer {
  ...
  async saveState() {
    ...
    if (logger) logger.logStorage('Saving player state', state);
  ...
  async loadState() {
    try {
      ...
      if (result.playerState) {
        ...
        if (logger) logger.logStorage('Player state loaded', { state });
        ...
        if (logger) logger.logStorage('No saved state found');
        console.log('[9Anime Playlist] ⚠️ No saved state in storage');
      }
    } catch (e) {
      if (logger) logger.logError(e, { context: 'loadState' });
  ...
  extractEpisodes() {
    ...
    if (logger) {
      logger.logPerformance('Episodes extracted', {
  ...
  setupControls() {
    ...
    document.getElementById('toggle-autoplay').addEventListener('change', (e) => {
      ...
      if (logger) logger.logUserInteraction('Autoplay toggled', { enabled: this.autoplay });
      this.saveState();
    });

    document.getElementById('toggle-loop').addEventListener('change', (e) => {
      this.loop = e.target.checked;
      if (logger) logger.logUserInteraction('Loop toggled', { enabled: this.loop });
      this.saveState();
    });
    ...
    document.getElementById('apply-genres').addEventListener('click', () => {
      ...
      console.log('[9Anime Playlist] Selected genres:', this.selectedGenres);
      if (logger) logger.logUserInteraction('Genres selected', { genres: this.selectedGenres });
      document.getElementById('genre-selector').style.display = 'none';
  ...
  playEpisode(index) {
    ...
    if (logger) {
      logger.logUserInteraction('Episode selected', {
    ...
    if (logger) logger.logNavigation('Navigating to episode', { url: episode.url });
  ...
  async jumpToRandomAnime() {
    ...
    if (logger) logger.logNavigation('Jumping to random anime', { genre, url: genreUrl });

    try {
      console.log('[9Anime Playlist] 📡 Sending fetch request...');
      const startTime = performance.now();
      const response = await fetch(genreUrl);
      if (logger) logger.logApiCall('Genre page fetched', { genre, status: response.status, duration: `${(performance.now() - startTime).toFixed(2)}ms` });
      ...
      if (animeLinks.length > 0) {
        ...
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        if (logger) logger.logNavigation('Auto-jump navigation', { fromGenre: genre, toAnime: randomAnime, animeChoices: animeLinks.length });
...
if (isParentPage && window.location.pathname.includes('/watch/')) {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data.type === 'VIDEO_ENDED') {
      ...
      if (logger) logger.logVideoEvent('Video ended', { autoplay: player.autoplay, autoJump: player.autoJump, queueLength: player.randomEpisodeQueue.length });
